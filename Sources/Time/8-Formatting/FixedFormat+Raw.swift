...
extension FixedFormat {
    ...
    /// Create `FixedFormat` value using hard-coded format string.
    ///
    ...
    public init(raw rawFormatString: String, strict: Bool = true) throws(TimeError) {
        ...
        let representedUnits = Fixed<Granularity>.representedComponents
        let missingUnits = Set(requiredUnits).subtracting(representedUnits)
        ...
        if missingUnits.isEmpty == false {
            if strict == true {
                ...
                let desc = "The provided format string '\(rawFormatString)' includes components (\(missingUnits)) that are not represented in a \(Fixed<Granularity>.self) value (\(representedUnits))"
                throw TimeError.invalidFormatString(rawFormatString, units: missingUnits, description: desc)
            } else {
                config = .raw(rawFormatString)
            }
        } else {
            config = .raw(rawFormatString)
        }
...
extension Fixed {
    ...
    public func format(raw rawFormatString: String, strict: Bool = true) throws(TimeError) -> String {
        let format = try FixedFormat<Granularity>(raw: rawFormatString, strict: strict)
